...
public class FootballRanker {
	...
	private double printRootMeanSquareError(List<String> humanRanks, TreeSet<AllPathsInfo> paths, boolean showResults) {
		...
		int graphRank = 1;
		boolean found = false;
		for (String teamName : humanRanks) { // go through all the teams
			for (AllPathsInfo p : paths) { // go through all our paths
				if (p.getName().equals(teamName)) { // if our path name matches the current AP team
					printInformation(showResults, teamName, apRank, graphRank, 0, 1);
					totalSquaredDiff += Math.pow(apRank - graphRank, 2); // update the total squared difference
					found = true;
				}
				graphRank++;
			}
			if (!found) { // if we never find the AP team in our paths, update the total squared diff the way specified in the assignments
				printInformation(showResults, teamName, apRank, graphRank, 0, 2);
				totalSquaredDiff += Math.pow(apRank - paths.size() + 1, 2);
			}
			apRank++;
			graphRank = 1; // reset our rank
		}
		double result = Math.sqrt(totalSquaredDiff / humanRanks.size());
		printInformation(showResults, "", apRank, graphRank, result, 3);
		return (double) Math.round(result * ROUNDED) / ROUNDED;
	}

    // this helper method executes all lines of code that involve printing out to the system
    private void printInformation(Boolean showResults, String name, int apRank, int graphRank, double result, int printStatement) {
		if (showResults) {
			if (printStatement == 1) {
				System.out.printf("%-20s - actual rank: %-1s predicted rank: %-1s\n", name, apRank, graphRank);
