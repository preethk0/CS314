...
public class Graph {
	...
	public void dijkstra(String startName) {
		...
		Vertex startVertex = vertices.get(currentStartVertexName); // get start vertex
		startVertex.weightedCostFromStartVertex = 0; // since it is the start vertex we modify these values
		startVertex.numEdgesFromStartVertex = 0;
		Path startPath = new Path(startVertex, startVertex.weightedCostFromStartVertex); // create a new path and add the start vertex to it
		startPath.add(startVertex);
		PriorityQueue<Path> pathQueue = new PriorityQueue<>();
		pathQueue.add(startPath); // put it in the queue
		dijkstraHelper(pathQueue);
	}
    
	private void dijkstraHelper(PriorityQueue<Path> pathQueue) {
		while (!pathQueue.isEmpty()) {
			Path currPath = pathQueue.remove(); // dequeue
			Vertex currVertex = currPath.dest; // we take current vertex as the one at the end of the path
			if (currVertex.scratch == 0) {
				currVertex.scratch = 1; // visited
				for (Edge e : currVertex.adjacent) { // iterate through its adjacent vertices
					if (e.cost + currVertex.weightedCostFromStartVertex < e.dest.weightedCostFromStartVertex) { // if we found a shorter path
						e.dest.weightedCostFromStartVertex = e.cost + currVertex.weightedCostFromStartVertex;
						e.dest.prev = currVertex; // we add this vertex to the path
						e.dest.numEdgesFromStartVertex = currVertex.numEdgesFromStartVertex + 1;
						Path newPath = new Path(e.dest, e.dest.weightedCostFromStartVertex); // then add it to the queue
	...
	public void findAllPaths(boolean weighted) {
		...
		for (Vertex v : vertices.values()) { // iterate through all vertices in the graph
			Vertex currVertex = v;
			currentStartVertexName = currVertex.name;
			if (weighted) { // apply the necessary algorithm
				dijkstra(currVertex.name);
			} else {
				findUnweightedShortestPath(currVertex.name);
			}
			int numVertexConnected = 0;
			double countWeighted = 0;
			double countUnweighted = 0;
			double currWeightedPathLength = 0;
			longestPathLength = updatePathInfo(weighted, longestPathLength, currVertex, numVertexConnected, countWeighted, countUnweighted, currWeightedPathLength);
		}
	}
	
	// this helper method does the actual updating of path information for each path from the start vertex
	private double updatePathInfo(boolean weighted, double maxPathLength, Vertex currVertex, int numVertexConnected, double countWeighted, double countUnweighted, double currWeightedPathLength) {
		for (Vertex adjVertex : vertices.values()) { // iterate through all vertices that the curr vertex has a path to
			if (adjVertex.weightedCostFromStartVertex != INFINITY && !adjVertex.equals(currVertex)) {
				countWeighted += getPath(adjVertex.name).weightedCostOfPath; // sum up weighted and unweighted values
				countUnweighted += getPath(adjVertex.name).getNumEdgesInPath();
				currWeightedPathLength = getWeightedCostFromStart(adjVertex.name);
				numVertexConnected++;
			}
			if (weighted) { // depending on if it is weighted, find the longest shortest path
				if (currWeightedPathLength > maxPathLength) {
					maxPathLength = getPath(adjVertex.name).weightedCost();
					longest = getPath(adjVertex.name);
				}
			} else {
				if (getPath(adjVertex.name).getNumEdgesInPath() > maxPathLength) {
					maxPathLength = getPath(adjVertex.name).getNumEdgesInPath();
					longest = getPath(adjVertex.name);
				}
			}
		}
		countUnweighted = weighted ? countUnweighted : countWeighted;
		vertices.get(currVertex.name).totalWeightedPathLength = countWeighted; // set the values
		vertices.get(currVertex.name).totalUnweightedPathLength = countUnweighted;
		currVertex.numVertexConnected = numVertexConnected;
		clearAll();
