...
public class BinarySearchTree<E extends Comparable<? super E>> {
	...
	private BSTNode<E> addHelper(BSTNode<E> n, E value) {
	...
	private BSTNode<E> removeHelper(BSTNode<E> n, E value) {
		if (n == null) { // when we reach a dead end we just return the current node
			...
			if (n.getLeft() == null && n.getRight() == null) { // if the node has no children
				...
				E minOfRightSubTree = minOfSubTree(n.getRight()); // we get the minimum of the right sub-tree
	...
	private boolean isPresentHelper(BSTNode<E> n, E value) {
	...
	private List<E> getAllHelper(BSTNode<E> n, ArrayList<E> elementList) {
	...
	private E minOfSubTree(BSTNode<E> n) {
		E min = null;
		while (n != null) {
			min = n.getData();
	...
	public boolean iterativeAdd(E data) {
		...
		if (root == null) { // if the tree is is empty, then we immediately set our root to the new node
			size++;
			root = newNode;
			return true;
		}
		...
		addNode(data, tempParentNode, newNode);
		
		return true;
	}	
	
	// this helper method actually adds the node to the tree in the position that we determined in the prev method
	private void addNode(E data, BSTNode<E> tempParentNode, BSTNode<E> newNode) {
		int direction = data.compareTo(tempParentNode.getData()); // now we set temp's parent left or right reference to that spot depending on compareTo
		if (direction < 0) {
	...
	private List<E> getAllLessThanHelper(BSTNode<E> n, ArrayList<E> elementList, E value) {
	...
	private List<E> getAllGreaterThanHelper(BSTNode<E> n, ArrayList<E> elementList, E value){
    	if(n == null) { // if we hit a null node, we just do nothing
    		return elementList;
    	}
	...
	private int numNodesAtDepthHelper(BSTNode<E> n, int d, int currDepth) {
		...
		if (d == currDepth) { // this means our current node is at the necessary depth, so we increment our count
			numNodesAtDepth++;
		}
		numNodesAtDepth += numNodesAtDepthHelper(n.getLeft(), d, currDepth + 1) + numNodesAtDepthHelper(n.getRight(), d, currDepth + 1);
		// our total count is the sum of the number of nodes at the depth from the left and right sub-trees
